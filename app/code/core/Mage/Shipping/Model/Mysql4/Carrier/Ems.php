...
class Mage_Shipping_Model_Mysql4_Carrier_Ems extends Mage_Core_Model_Mysql4_Abstract
{
	protected function _construct()
	{
		$this->_init('shipping/ems', 'pk');
	}
...
//                     OR (dest_country_id='0' AND dest_region_id='0' AND dest_zip='')");
	 ...
	 * @return bool|Mage_Shipping_Model_Mysql4_Carrier_ems
	 */
	public function uploadAndImport(Varien_Object $object)
	{
		...
		$csvFile = $_FILES['groups']['tmp_name']['ems']['fields']['import']['value'];

		if (!empty($csvFile)) {

			$csv = trim(file_get_contents($csvFile));

			$table = Mage::getSingleton('core/resource')->getTableName('shipping/ems');
			...
			*/
			...
			if (isset($_POST['groups']['ems']['fields']['condition_name']['inherit'])) {
				$conditionName = (string)Mage::getConfig()->getNode('default/carriers/ems/condition_name');
			} else {
				$conditionName = $_POST['groups']['ems']['fields']['condition_name']['value'];
			}
...
//            } else {
//                $conditionName = $websiteModel->getConfig('carriers/ems/condition_name');
//            }
			$conditionFullName = Mage::getModel('shipping/carrier_ems')->getCode('condition_name_short', $conditionName);
			if (!empty($csv)) {
				$exceptions = array();
				$csvLines = explode("\n", $csv);
